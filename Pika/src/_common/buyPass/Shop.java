...
public class Shop {
		...
		String name; // 사용자명
		String ename; // 사용자 아이디
		String empno; // 사용자 비밀번호
		String product; // 상품
	...
	String login(String ename, String empno) {
		...
		if (ename.equals(ename) && empno.equals(empno)) {
			this.ename = ename;
			this.empno = empno;
			result = "로그인 성공";
		} else {
			this.ename = "로그인 되지 않음";
			this.empno = "로그인  실패";
		}
	...
	void buy(String product) {
		...
		if (ename == null || ename.equals("로그인 되지 않음")) {
	...
	void current() {
		...
		System.out.println("로그인한 계정 : " + ename);
	}
